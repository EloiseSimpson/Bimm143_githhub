...
analysis we have learned thus far - including the machine learning
...
analysis of this dataset - we don’t want to pass this to PCA etc. We wil
sve it as a seperate wee vector that we can use later to compare our
...
columns/features/dimensions are in very different scales in the original
data set.
...
> captured by the first principal component (PC1)? 44.27% Q5. How many
> principal components (PCs) are required to describe at least 70% of
> the original variance in the data? Three Q6. How many principal
> components (PCs) are required to describe at least 90% of the original
> variance in the data? Seven

``` r
biplot(wisc.pr)
```

![](class08_miniproject_files/figure-commonmark/unnamed-chunk-16-1.png)

> Q7. What stands out to you about this plot? Is it easy or difficult to
> understand? Why?

It is very difficult to understand because all the data nad labels are
overlapping so nothing can be read/ discerned.
...
M values are separated from B values by PC1. B and M are also less
segregated in the second one.
...
![](class08_miniproject_files/figure-commonmark/unnamed-chunk-25-1.png)
...
ward.D2 because it puts the B and M values in different clusters to the
greatest extent.

## Combining methods

The idea here is that I can take my new variables (i.e. the PCs) that
are better descriptors of the data-set than the original features
(i.e. the 30 columns in `wisc.data`) and use these as a basis for
clustering.
...
![](class08_miniproject_files/figure-commonmark/unnamed-chunk-31-1.png)
...
Again, pretty well, the Benign diagnoses are more accurate, but the
malignant diagnoses are less accurate.

## Sensitivity

Our cluster “1” has 24 benign and 179 malignant, cluster “2” has 333
benign and 33 malignant.

179 TP (true positives) 24 FP 333 TN (true negative) 33 FN
...
![](class08_miniproject_files/figure-commonmark/unnamed-chunk-38-1.png)
